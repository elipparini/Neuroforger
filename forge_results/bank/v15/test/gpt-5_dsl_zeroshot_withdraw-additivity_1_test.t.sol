...
// concretization for abstract contracts[] cs;
contract Dummy {}
...
contract BankTest is Test {
    ...
    function test_withdraw_additivity_violation() public {
        // concretization for abstract transaction[] txs;
        vm.deal(address(0xBEEF), 1 ether);
        vm.prank(address(0xBEEF));
        bank.deposit{value: 1 ether}();

        // choose the sender who deposited
        address sender = address(0xBEEF);
        uint256 snapshot = vm.snapshotState();

        // measure effects on the same user (the sender)
        address user = address(0xBEEF);
        
        // choose amounts: n1 <= deposit, but n1 + n2 > deposit
        uint256 n1 = 0.6 ether;
        uint256 n2 = 0.5 ether;
        bool revert_pathA1;
        bool revert_pathA2;
        bool revert_pathB;
